...
## Project Description
 ...
 >   * **Strategy**: The strategy pattern will be used within our exercise and meal plan subclasses. The JSON system will be implemented from our HealthPlan parent class and will be used with in the Meal and Exercise plan subclasses. The JSON system will help implement the API Caller and Handler. We switched from Vistor to Strategy because the implementation of the Strategy Pattern was planned to be used by the group for those subclasses. So we decided to not complicate our project with including the Visitor Pattern as well.
